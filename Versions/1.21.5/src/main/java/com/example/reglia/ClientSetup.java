...
public class ClientSetup {
    ...
    public static void onClientChatReceived(net.neoforged.neoforge.client.event.ClientChatReceivedEvent event) {
        ...
        if (m.find()) {
            ...
            // Calculate newlines based on height hint, or default to smaller size
            int height = 64; // Default for 1.21.4 (smaller to fit chat)
            if (heightHint != null) {
                height = Math.min(Integer.parseInt(heightHint), 64);
            }
            int lines = (int) Math.ceil(height / 10.0); // ~6-7 lines for 64px
            String spacing = "\n".repeat(lines);
