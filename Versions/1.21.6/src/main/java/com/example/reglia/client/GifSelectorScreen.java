...
public class GifSelectorScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        // 1.21.6 strict blur fix: Use manual background fill instead of renderBackground
        ...
        for (int i = 0; i < gifs.size(); i++) {
            ...
            if (frame != null && anim != null && anim.width > 0 && anim.height > 0) {
                // 1.21.4+ uses PoseStack for scaling
                // Calculate scale to fit GIF into 64x64 cell
                float scaleX = (float) size / anim.width;
                float scaleY = (float) size / anim.height;
                float scale = Math.min(scaleX, scaleY); // Uniform scale to maintain aspect ratio

                graphics.pose().translate(x, y);
                graphics.pose().scale(scale, scale);

                // Render at origin (0,0) since we translated, at full texture size
                graphics.blit(frame, 0, 0, 0, 0, anim.width, anim.height, anim.width,
                        anim.height);
